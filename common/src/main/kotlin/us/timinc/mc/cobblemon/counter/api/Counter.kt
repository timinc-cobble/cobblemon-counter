...
class Counter(
    ...
    fun decode(buf: RegistryFriendlyByteBuf) {
        streak.species = buf.readString().asIdentifierDefaultingNamespace(CounterMod.modId)
        ...
        for (i in 0 until countSize) {
            val speciesId = buf.readString().asIdentifierDefaultingNamespace(CounterMod.modId)
